...
```{r}
subset(data, name == "Agaricales", supertaxa = TRUE) %>%
  plot(vertex_size = item_count,
       vertex_color = item_count,
       vertex_label = name)
```
...
part_c <- subset(data, name == "Pleosporales")
sum(part_a, part_b, part_c) %>%
  plot(vertex_size = item_count,
       vertex_color = item_count,
       vertex_label = name,
       tree_label = name,
...
Note that if the content of the parts summed overlap, this does not work yet. 
We might need to have more formal item ids to make this work..

```{r, eval = FALSE}
part_d <- subset(data, name == "Strophariaceae")
sum(part_a, part_b, part_c, part_d) %>%
  plot(vertex_size = item_count,
       vertex_color = item_count,
       vertex_label = name,
       tree_label = name,
...
```{r}
subset(data, , sample(nrow(data$item_data), 50)) %>%
    plot(vertex_size = item_count,
         vertex_color = item_count,
         vertex_label = name)
```

By default, even taxa with no items are preserved.

```{r}
subset(data, name == "Ascomycota", sample(nrow(data$item_data), 100)) %>%
    plot(vertex_size = item_count,
         vertex_color = item_count,
         vertex_label = name)
```
...
But these can be removed using the `itemless` option.

```{r}
subset(data, name == "Ascomycota", sample(nrow(data$item_data), 100), itemless = FALSE) %>%
    plot(vertex_size = item_count,
         vertex_color = item_count,
         vertex_label = name)
```
