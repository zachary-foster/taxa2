...
<img src="taxa_class_ideas.png" title="taxa classes diagram" width="718">
...
These IDs are better than using the taxon name directly because they are guaranteed to be unique, whereas there are often duplicates of taxon names. 
...
To make manipulation easier, there are methods for `taxomomy` that can provide indexes in a taxonomy context.
...
It is basically the same as the `taxonomy` class, but with the following additions:
  ...
  * A list called `funcs` that store user defined functions
...
To see what variables can be used this way, use `all_names` or `get_data`.
...
For example, note how "taxon_names" and "dangerous" is used below. 
