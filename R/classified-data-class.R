...
#'
#' @export
item_data <- function(obj, calculated_cols = TRUE) {
  UseMethod("item_data")
}

#' @export
item_data.classified <- function(obj, calculated_cols = TRUE) {
  data <- cbind(data.frame(taxon_id = obj$item_taxon_id),
                obj$item_data)
  #   if (calculated_cols) {
  #     calculated_data <- lapply(obj$taxon_funcs, item_apply, obj = obj)
  #     names(calculated_data) <- names(obj$taxon_funcs)
  #     data <- cbind(data, as.data.frame(calculated_data))
  #   }
  return(data)
